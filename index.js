...
//   let scByCID = results.filter(item => item.order.customerId == "ERNSH");
  
//   console.log(scByCID.length);
  
  
//   "3) employeeİd- görə sortlayın"
  
//   let scByEID = results.filter(item => item.order.employeeId);
  
//   let scByEIDS = scByEID.sort((a, b) => a.order.employeeId - b.order.employeeId);
  
//   console.log(scByEIDS);
  
  
//   "4) OrderDetailsi 3-dən çox olanları console-a yazdır"
  
//   let scByOrder = results.filter(item => item.orderDetails.length > 3);
  
//   console.log(scByOrder);
  
  
//   "5) İD-si 11075 olanı tap və yazdır. (find)"
  
//   let scByID = results.filter(item => item.order.id == 11075)
